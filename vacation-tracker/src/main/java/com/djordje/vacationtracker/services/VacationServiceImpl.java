...
public class VacationServiceImpl implements VacationService {
    ...
    public String insertVacation(MultipartFile file) {
        int totalAdded = 0;
        List<String[]> vacationData = null;

        if (!csvVacationReader.csvExtensionCheck(file))
            return "File must be .csv!";
        try {
            vacationData = csvVacationReader.readVacationDates(file);
        } catch (Exception e) {
            return null;
        }
        for (String[] line : vacationData) {
            String email = line[0];

            if (!employeeRepository.existsById(email)) {
                continue;
            }
            Date startDate = dateUtils.convertStringToDate(line[1]);
            Date endDate = dateUtils.convertStringToDate(line[2]);
            java.sql.Date sqlStartDate = new java.sql.Date(startDate.getTime());
            java.sql.Date sqlEndDate = new java.sql.Date(endDate.getTime());
            vacationRepository.save(new Vacation(email, sqlStartDate, sqlEndDate));
            totalAdded++;
        }
        if (totalAdded > 0) {
